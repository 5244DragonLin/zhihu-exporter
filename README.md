...
**zhihu_exporter 解决这些问题**：输入用户主页链接，自动爬取所有点赞回答和文章，每条内容保存为一个格式优美的 Markdown 或 HTML 文件。同时支持直接输入专栏链接，一键导出专栏全部文章。

## ⭐亮点

- **三种导出目标**：点赞过的他人内容（`--mode upvotes`，默认）、作者自己创作的文章 / 回答（`--mode articles` / `answers` / `all`，免浏览器、免签名）、专栏全部文章（`--mode column`，链接自动识别，公开专栏免 Cookie），命令行入口统一
- **双格式输出**：Markdown 纯文本 + HTML 富文本（保留原始排版、图片、样式）
- **增量导出**：`.progress.json` 记录断点，下次只抓新增内容，无新增时秒级完成、不重扫统计报告
- **图片下载**：图片可下载到本地或上传 Gitee 图床，正文图片链接自动替换，避免知乎图床失效
- **领域分类**：统计报告按 13 大领域自动归并知乎官方标签（影视文艺、两性婚恋、科技互联网、时事政治、教育成长等），点击领域可展开查看该领域全部条目；冷门问题可在 `categories.py` 手动指定归属
- **拟人化策略**：请求间隔 + 限流重试，降低风控风险
- **多种登录**：Cookie 字符串 / Cookie 文件 / Playwright 扫码登录，任选其一
...
复制 `config.example.yaml` 为 `config.yaml`，按需填写 Cookie、图床等参数（字段说明见「配置文件」章节）。不传 `--config` 时会自动读取当前目录的 `config.yaml`。
...
作者原创、专栏等更多导出场景见「导出模式」章节。

## 📦导出模式

三种导出目标共用同一个命令行入口，靠 **URL 类型** 或 `--mode` 区分：

| 导出目标 | 触发方式 | 免浏览器 / 免签名 |
|----------|----------|-------------------|
| 点赞内容 | 用户主页 URL（`--mode upvotes`，默认） | 否，需 Cookie 或扫码登录 |
| 作者原创 | 用户主页 URL + `--mode articles` / `answers` / `all` | 是，但仍需 Cookie |
| 专栏文章 | 专栏 URL（自动识别）或 `--mode column` | 是，**公开专栏免 Cookie** |

### 点赞导出（默认）

导出指定用户**点赞过的**他人回答与文章（输出结构见「效果预览」）：
...
# 只导出点赞的回答 / 文章
python zhihu_exporter.py https://www.zhihu.com/people/xxx --only-answers
python zhihu_exporter.py https://www.zhihu.com/people/xxx --only-articles
```

### 作者原创导出

导出指定用户**自己创作**的文章 / 回答（免浏览器、免签名）：
...
导出指定**知乎专栏**的全部文章（免浏览器、免签名）：

> 公开专栏无需 Cookie，直接以游客模式抓取；付费 / 需登录专栏请用 `-c` 或 `-C` 提供 Cookie。
...
```text
python zhihu_exporter.py [用户主页URL | 专栏URL] [选项]
```
...
- **重大变更：** 项目从 zhihu_upvote_exporter 更名为 zhihu_exporter，包名、目录、仓库名全部同步更新
- **新增：** --mode articles / answers / all 作者原创导出（免浏览器、免签名），支持 --keyword 标题关键词过滤
- **新增：** --mode column 专栏导出：输入专栏链接自动识别并导出专栏全部文章（免浏览器、免签名；公开专栏免 Cookie，无需登录）
- **新增：** column.py 专栏导出模块，走 /api/v4/columns/{column_id}/articles 接口，正文内嵌无签名校验
- **新增：** 封面提取：文章用 API image_url、回答提取正文首图，Markdown 文末追加 ![cover]、HTML 展示封面
- **新增：** request.delay / request.max_retries 配置项——翻页间隔与重试次数从源码常量提为可配置
- **优化：** 专栏文章文件名仅使用标题，无日期/ID 前后缀，简洁易读
- **优化：** 增量模式下已存在的文件直接跳过；统计报告分组与手动覆盖表匹配提速
- **变更：** 配置键对齐《爬虫项目指南》统一规范
- **修复：** 配置文件缺失 page_delay / max_retries 时报错（getattr 安全取值）
- **修复：** 中途中断/请求失败不再丢数据，扫码登录真正等待登录成功
- **修复：** 不传 --config 时未自动加载 config.yaml 的问题；--concurrency 真正限制全部请求并发
